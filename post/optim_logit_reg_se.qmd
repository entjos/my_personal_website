...
[R Code](/rcode/optim_logit_reg_se.txt){.btn .btn-outline-primary .btn role="button"}
...
Combining this and my previous post on <a href="/post/optim_logit_reg.html">optimizing a logistic regression using optimx()</a>, we were able to more or less manually obtain the results of a logistic regression model, that we would commonly obtain using the `glm()` function.
