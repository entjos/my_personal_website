...
where $Z(W)$ is the set if $Z$-nodes that are not ancestors of any $W$-node in $G_\overline{X}$.
...
Please not that we used $x'$ in @Eq-dag-d-4 in order to differentiate between the $x$ in $\do(x)$ and the $x$ observed in our dataset. The second part of @Eq-dag-d-4 means a summation over all observed values of $X$ independetly of the value that is chosen for $\do(x)$.

[#here](../pictures/3_rules_of_do_calculus_dag_2.png)
